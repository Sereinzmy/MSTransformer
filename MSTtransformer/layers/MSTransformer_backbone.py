__all__ = ['MSTransformer_backbone']
...
# Cell
class MSTransformer_backbone(nn.Module):
...
class TSTEncoder(nn.Module):
    ...
    def forward(self, src: Tensor, key_padding_mask: Optional[Tensor] = None, attn_mask: Optional[Tensor] = None):
        ...
        if self.res_attention:
            for mod in self.layers: output, scores = mod(output, prev=None, key_padding_mask=key_padding_mask,
                                                         attn_mask=attn_mask)
            return output
        else:
...
class TSTEncoderLayer(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=256, store_attn=False,
        ...
        self.out_fc = nn.Linear(self.seq_length // self.scale_1 + self.seq_length // self.scale_2 + self.seq_length,
                                self.seq_length)
    ...
    def forward(self, src: Tensor, prev: Optional[Tensor] = None, key_padding_mask: Optional[Tensor] = None,
        ...
        if self.res_attention:
            ...
            src_scale_1, attn, scores = self.self_attn(next_input_1, next_input_1, next_input_1, prev,
                                                       key_padding_mask=key_padding_mask, attn_mask=attn_mask)
